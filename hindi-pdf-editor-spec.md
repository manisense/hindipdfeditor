...
- **Optional next step (not started)**: opt-in per-page "Enhance with AI" cloud OCR (Gemini free tier) for scans where on-device accuracy isn't enough — strictly opt-in because it sends page images off-device.
