...
### Phase 4.6 — Stable editor viewport and asset-led UX — IMPLEMENTED, device verification pending
...
- `PdfPageViewer.tsx` now nests horizontal and vertical native scroll surfaces around the same zoomed page, so a zoomed page pans on both axes. Pinch focal-point math clamps against the real page content dimensions and the measured viewport.
...
- A focused overlay displays a dedicated **Move** handle. Dragging previews locally and commits a new PDF-point `xPt`/`yPt` only at gesture end, with one undo checkpoint for the whole move.
...
- **Verification status:** TypeScript, ESLint, unit tests, Prettier, and Android Metro bundling can be run locally. No Android device was attached during implementation, so keyboard/pan/drag behavior and the required real exported-PDF visual check must be completed on a physical device before this phase is marked passed.
