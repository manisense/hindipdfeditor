...
**Status:** Phases 0 through 4.6 passed on a real physical Android device (Section 10). Phase 4.7 is implemented and locally verified: it hardens safe-area layout, point-sized per-page export, export quality, redo, blank-page authoring, File Manager saving, and explicit legacy-font Unicode replacement. Its compositor has been checked with the canonical Devanagari fixture in independent desktop PDF renderers; the native Android raster-quality and Storage Access Framework paths still require the post-build physical-device pass before Phase 4.7 can be marked passed. Phase 5 (direct glyph injection) remains explicitly deferred per AGENTS.md and is not started. This document is self-contained and is the primary implementation brief.
...
1. The specific PDF page being edited is rasterized at 3× point dimensions to a high-quality JPEG using the OS native PDF renderer in print mode. This becomes an immutable visual content layer. PNG was the original choice, but a full-page PNG combined with shaped Devanagari text hung Android's print WebView on a real device. JPEG quality 97 retains the opaque page detail while keeping that resolved encoding path.
...
3. At save/export time, one point-sized HTML document is assembled per page. Its base64-inlined `<img>` content layer and escaped edits use the same canonical PDF-point coordinate system and real embedded Devanagari fonts.
4. Each isolated page is exported through Android's native print pipeline (`WebView.createPrintDocumentAdapter()`), validated for page count and dimensions, then copied into the final multi-page PDF with `@cantoo/pdf-lib`. The library only copies already-shaped pages; it never draws Devanagari strings.
...
| Devanagari font                                    | Noto Sans Devanagari bundled; Mukta downloadable on demand                                           | immutable files from `google/fonts` (OFL license) | Noto Sans is the offline-safe default. Mukta is fetched only from a pinned Google Fonts commit, checked for exact byte size and OpenType signature, loaded into the editor, and embedded into export. Font candidates are admitted only after the canonical Devanagari fixture renders cleanly through independent PDF rasterizers.                                                                                                                                                                                                   |
...
/src
  ...
  /lib
    ...
    exportPdf.ts             // prints one exact-size page at a time, validates, then merges
    savePdf.ts               // Android SAF directory picker + validated persistent copy
    blankPage.ts             // immutable cached white background for inserted pages
    ...
    fontAsset.ts             // validates/loads bundled and curated downloadable fonts
  /state
...
type TextEdit = {
  ...
  text: string;
  color: string; // hex
  fontFamily: 'NotoSansDevanagari' | 'Mukta';
  widthPt?: number; // optional fixed wrapping width, in PDF points
  replacement?: { maskId?: string; ocrLine?: OcrLine }; // undoable pair metadata
};
...
Builds exact point-sized print HTML. The page, mask rectangles, edit positions, wrapping widths, and font sizes all use CSS `pt`, so changing raster resolution cannot scale or rotate edits. The base64 image is an absolutely positioned `<img>` content layer. Text explicitly uses horizontal writing mode, normal word breaking, and escaped user content; masks render before text.

```typescript
function pageHtml(page: PageState, backgroundImageDataUrl: string): string {
  // Render masks first and escaped text second, all in canonical PDF points.
  return `<section style="position:relative;width:${page.widthPt}pt;height:${page.heightPt}pt;overflow:hidden"><img src="${backgroundImageDataUrl}" style="position:absolute;inset:0;width:100%;height:100%" />${layers}</section>`;
}
```
...
Calls `Print.printToFileAsync({ html, width, height })` once per page with that page's exact point size. It rejects extra/blank sheets and size drift, merges the validated one-page outputs with `copyPages`, then verifies the merged file is non-empty, parseable, has the expected page count, and preserves every page size. Mixed-size documents and inserted blank pages therefore use their own dimensions. The original source is never modified.

### `savePdf.ts`

Uses Android's Storage Access Framework directory picker to create a new PDF in a user-selected File Manager folder. It copies rather than moves the validated cache export, parses the persisted copy back, and deletes a failed destination. Sharing/opening remains a separate action.

### `legacyFontDetector.ts`

On document load, use `@cantoo/pdf-lib`'s `PDFDocument.load()` to enumerate embedded font names across all pages. Match against known legacy Devanagari font name patterns: `KrutiDev*`, `Shivaji*`, `Chanakya*`, `DevLys*`, `Walkman-Chanakya*`, `Agra*`, `Amar*`. If inspection fails or is inconclusive, mark the page unknown and block editing without bypass. A positively identified legacy page may enter explicit raster-only Unicode replacement mode after warning and confirmation: the burned-in source stays untouched and every new edit uses a verified Unicode font. Never treat downloading the legacy font as decoding its non-Unicode text.
...
- [x] Tap-to-edit: tapping a detected line consumes it (highlight gone, can't double-mask), masks its region via the same shared `maskAndReplaceRegion` path Phase 3's manual flow now also uses, and opens an auto-focused `TextInput` pre-filled with the OCR text. Phase 4.6 sizes it at `1.08 ×` the OCR ink-box height (6pt floor) so the nominal font size no longer appears consistently smaller than the original; its width remains constrained to the detected line (`TextEdit.widthPt`) so live view and export wrap identically. Tap on empty page still adds new text (Phase 1 behavior); manual drag-to-mask remains the fallback for OCR misses.
...
- [x] **Opt-in per-page AI OCR through the production proxy.** The Android app never receives a
      Gemini credential. After explicit consent it sends one page image to
      `https://api.hindipdfeditor.com/v1/ocr`; the Worker holds the Gemini key as a secret, requests
      non-stored output, validates normalized boxes, and returns the same engine-neutral line contract
      used by on-device OCR. The original PDF stays local and immutable.
- [x] **Bidirectional production translation.** The editor offers Hindi → English and English →
      Hindi for the current page or whole document. It sends only detected source lines with stable
      IDs through `/v1/translate`, validates target-script results, and applies mask + Unicode overlays
...
- Replaced the page-length outer editor scroll with a fixed workspace. When the keyboard opens, nonessential editor chrome collapses once and the selected edit is revealed inside the page viewport; typing no longer causes the whole document layout to recalculate on every letter.
- `PdfPageViewer.tsx` now nests horizontal and vertical native scroll surfaces around the same zoomed page, so a zoomed page pans on both axes. Pinch focal-point math clamps against the real page content dimensions and the measured viewport. Both Android scroll surfaces set `scrollsChildToFocus={false}`: the app reveals an edit once on focus, then ignores Android's transformed caret-reveal requests that previously moved the page after each controlled text update.
- New and manually replaced text always receives a stable `widthPt`; the live `TextInput` disables internal scrolling and font scaling, preventing per-character intrinsic-size changes and keeping the caret anchored.
- OCR replacements use `fontSizeForOcrLine()` (`1.08 ×` visible OCR-box height, 6pt floor) instead of the previous undersized ratio. `textEditGeometry.test.ts` covers font sizing plus text-box width and page-edge clamping.
- A focused overlay displays a dedicated **Move** handle. Its persistent capture-first `PanResponder` owns the gesture before either native page scroller can intercept it; dragging previews locally and commits a new PDF-point `xPt`/`yPt` only at gesture end, with one undo checkpoint for the whole move.
- The phone/tablet layout now follows the supplied assets: branded header, blue active tools, soft-gray page workspace, compact feature/mode chips, top-level export action, and a wide-screen text-properties panel. Existing Edit, OCR, AI enhancement, Translate, undo, paging, export, and legacy-font safety features remain present.
- The unloaded home screen uses the same asset-led visual language: Hindi greeting, an explicit PDF-picker entry card, and compact Edit, Translate, and OCR shortcuts. The shortcuts intentionally all use the supported document-open path; merge, split, and compression are not represented as if they existed.
- **Verified on-device (Nothing A142, Android 16):** `npm run android` produced a successful native build and installed on the attached phone. With the canonical fixture open, selecting an OCR edit kept it visible above Gboard; an 8-second device recording stayed on the exact same page region while `a`, `b`, and `c` were entered. A handle drag from `[433,736–597,800]` to `[618,868–783,931]` moved the text input by the same 185×132px delta. A temporary debug-only 200% initial zoom (removed immediately after the test) allowed independent one-finger horizontal and vertical pan verification using the production nested scroll surfaces.
- **Font/export visual verification:** selecting the fixture's large `धर्म` word produced a 33pt pre-filled overlay whose live glyph height matched the burned-in source and whose reph remained correctly shaped. The earlier moved edit exported through Plan A to a new 98,465-byte PDF; the pulled file parsed as one 612×792pt page, `pdftotext` extracted `abc`, and a 150-DPI `pdftoppm` render showed the moved overlay at the live-view location with the fixture's conjuncts, matras, and reph intact. The device's picked source copy retained the fixture's exact SHA-256 (`6730d50f3bcf2ff5c61ba0ae4ded3a68cf1548940c89a429e4cb26c2423085b2`), confirming export did not mutate the original.

### Phase 4.7 — Reliable export and complete authoring flow — IMPLEMENTED, device verification pending

- [x] Apply Android safe-area insets to the landing/editor headers and bottom command region; wrap narrow command rows so Export and Redo cannot sit under system navigation or outside the viewport.
- [x] Print every page as an isolated point-sized HTML document, reject unexpected sheets or dimensions, merge the validated pages with `copyPages`, and parse the final PDF back. This removes the alternating blank-page failure and supports mixed-size/inserted pages without direct text drawing.
- [x] Render source pages at 3× in `PdfRenderer` print mode and JPEG quality 97. Export positions remain in CSS points, independent of raster size.
- [x] Force horizontal text writing and normal Devanagari word breaking in export, preventing narrow OCR replacement boxes from stacking shaped clusters vertically.
- [x] Add history-aware Redo, with the future branch cleared by every new mutation and replacement mask/text metadata stored inside the undo snapshot.
- [x] Allow direct typing on confirmed-empty pages and insertion of new white pages without changing the source document.
- [x] Add Android File Manager folder selection via Storage Access Framework, with parse-back validation of the persisted copy; keep Share/Open as a separate action.
- [x] Keep unknown font inspection blocked. Permit only positively identified legacy pages to enter explicit raster-only Unicode replacement mode after warning and confirmation.
- [x] Add a curated font picker. Noto Sans remains bundled; Mukta downloads from an immutable official Google Fonts commit and must pass exact-size/OpenType checks before editor use or export embedding. Each page embeds only the fonts it uses, with the file's real weight descriptor: variable Noto Sans is `100 900`, static Mukta Regular is `400`.
- [x] Unit/static checks cover per-page export, mixed page sizes, redo branching, blank-page creation, legacy policy, persisted-file validation, font validation, HTML escaping, and horizontal text CSS.
- [x] Desktop compositor fixture: real Devanagari pages with conjuncts/reph were printed and merged, then rendered independently with Poppler backends without blank/black pages or vertical text.
- [ ] Re-run the canonical fixture on the production Android build: visually compare 3× raster quality, a multi-page export, a blank inserted page, legacy safe replacement, downloaded Mukta, and File Manager persistence. This cannot be marked passed from a desktop Chromium fixture alone.
...
- A page detected as using a legacy font warns before edit; unknown encoding stays blocked, and known-legacy safe replacement never interprets the old byte encoding.
...
- **Very large documents now make one print call per page.** This bounds WebView image allocation and prevents cross-page blank sheets, then merges via `copyPages`, but it can increase total export latency. A 10+ page physical-device run remains required before release sign-off.
- **Versions are now pinned (Section 4.1, checked July 2026):** Expo SDK 56, `expo-print` matching SDK 56, `@cantoo/pdf-lib` 2.7.1, plus the in-house `pdf-page-image` and `text-recognition` modules. Re-check this table before starting a build far in the future.
